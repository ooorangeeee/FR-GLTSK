...
# 【excel】载入电子表格，记录实验结果
xlsx_file = rf'./result/{"FR_cla_results"}.xlsx'
...
datasets_file = r'../data_pt/low_classification/{}.pt'
dataset_name_list = [
    # "Iris",
    # "Appendicitis",
    # "Pima",
...
para_dict = {
    19: [0.1, 30, 50, 0.008, 0.04, 0.4, -0.3, 0.005, 600],
    20: [0.1, 30, 50, 0.008, 0.04, 0.4, -0.3, 0.005, 600],
    21: [0.1, 30, 50, 0.008, 0.04, 0.4, -0.3, 0.005, 600],
    # 22: [0.06, 30, 50, 0.008, 0.01, -0.05, -0.3, 0.01, 600],
    # 23: [0.06, 30, 50, 0.008, 0.01, -0.05, -0.3, 0.01, 600],
}
        ...
        for t in range(num_repeats):  # 对于数据集data_name，对于参数组合para_str，第t次重复实验
            ...
            for k, (tra_ind, test_ind) in enumerate(kf.split(sample, target)):  # 十折交叉实验，当前一折
                ...
                train_full_batch(model, tra_sam, tra_tar, learning_rate, epoch, optim_type="Adam", gpu=True,
                                 task="classification", coef_ant_lambda=coef_ant_lambda,
                                 coef_con_lambda=coef_con_lambda)
